...
import { LayoutDashboard, CheckSquare, Users, Settings, ScrollText, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { to: '/admin/panel', icon: Settings, label: 'Admin Panel' },
  ];
    ...
    <div className="flex h-screen bg-base overflow-hidden">
      ...
      <aside className={`flex flex-col bg-surface border-r border-border transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-border">
          ...
          {isCollapsed && <div className="w-6 h-6 bg-accent rounded-sm mx-auto"></div>}
            ...
            {!isCollapsed && (
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="px-1.5 py-0.5 bg-accent/20 text-accent rounded text-[10px] font-bold uppercase tracking-wider">
                    Admin
                  </span>
                </div>
              </div>
